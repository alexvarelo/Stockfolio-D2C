...
- `src/components/dashboard/StockyChat.tsx` (the AI chat panel) predates this design system and still uses raw Tailwind swatches (`gray-*`, `red-*`, `indigo-*`) instead of the semantic tokens (`muted-foreground`, `danger`, `card`, etc.) that the rest of the authenticated app — including the social feed in `CreatePost.tsx`/`DashboardPosts.tsx`, which already follows the token system — uses consistently. Off-ramp font sizes in this file were normalized to the nearest `{typography}` step (2026-08-02); the color palette still needs the same pass.
